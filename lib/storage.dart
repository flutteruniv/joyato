...
class AccountRepository {
  ...
  late final reference = firestore.collection(accounts);

  late final converter = reference.withConverter<Account>(
    fromFirestore: (ds, _) {
      return Account.fromDocumentSnapshot(ds);
    },
    toFirestore: (account, _) {
      return account.toJson();
    },
  );

  /// Documentsが格納されているか確認し関数を分岐して実行する
  Future<Account?> checkDocuments(
  ...
  Future<Account?> storeUserData(
    ...
    final userDocRef = converter.doc(uid);
  ...
  Future<Account?> fetchByUid(String uid) async {
    final docSnapshot = await converter.doc(uid).get();
